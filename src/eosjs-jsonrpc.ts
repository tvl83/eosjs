...
export class JsonRpc implements AuthorityProvider, AbiProvider {
	...
	/** FIO SPECIFIC RPC CALLS */
	public async get_fio_names(publicKey: string) {
		return await this.fetch('/v1/chain/get_fio_names', {fio_public_key: publicKey})
	}

	public async get_fio_addresses(publicKey: string) {
		return await this.fetch('/v1/chain/get_fio_addresses', {fio_public_key: publicKey})
	}

	public async get_fio_domains(publicKey: string) {
		return await this.fetch('/v1/chain/get_fio_domains', {fio_public_key: publicKey})
	}

	public async get_fio_balance(publicKey: string) {
		return await this.fetch('/v1/chain/get_fio_balance', {fio_public_key: publicKey})
	}

	public async get_fee(endpoint: string, fioAddress: string) {
		return await this.fetch('/v1/chain/get_fee', {end_point: endpoint, fio_address: fioAddress})
	}
} // JsonRpc
